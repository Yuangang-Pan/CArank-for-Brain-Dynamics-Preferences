...
for i in range(Training_FFT.size):
    ...
    Train_ord = np.hstack(((Pair_Train_ord[:,1]* Num_Channel +Pair_Train_ord[:,0]).reshape(-1,1),  (Pair_Train_ord[:,2]* Num_Channel +Pair_Train_ord[:,0]).reshape(-1,1)))
    Train_eql = np.hstack(((Pair_Train_eql[:,1]* Num_Channel +Pair_Train_eql[:,0]).reshape(-1,1),  (Pair_Train_eql[:,2]* Num_Channel +Pair_Train_eql[:,0]).reshape(-1,1)))

    Test_ord = np.hstack(((Pair_Test_ord[:,1]* Num_Channel +Pair_Test_ord[:,0]).reshape(-1,1),  (Pair_Test_ord[:,2]* Num_Channel +Pair_Test_ord[:,0]).reshape(-1,1)))
    Test_eql = np.hstack(((Pair_Test_eql[:,1]* Num_Channel +Pair_Test_eql[:,0]).reshape(-1,1),  (Pair_Test_eql[:,2]* Num_Channel +Pair_Test_eql[:,0]).reshape(-1,1)))
    ...
    for e in range(Num_epoch):
        e_losses += train_epoch(Train_BCI_feat, Train_ord, Train_eql, model, optimizer, criterion, flag_cuda, Batch_size, e)
    ...
    scipy.io.savemat('/Classfication (A)/Train_score_pres.mat', mdict={'Train_score_pres': Train_score_pres})
    scipy.io.savemat('/Classfication (A)/Test_score_pres.mat' , mdict={'Test_score_pres': Test_score_pres})
    scipy.io.savemat('/Classfication (A)/Acc.mat', mdict={'Acc': Acc})
    scipy.io.savemat('/Classfication (A)/RMSE.mat', mdict={'RMSE': RMSE})
