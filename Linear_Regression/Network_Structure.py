...
def MV_Baseline(Z_score, Pair):
    delta_Z = Z_score[Pair[:, 0]] - Z_score[Pair[:, 1]]
    delta_Z = np.clip(delta_Z, -50, 50)
    flag = 1 / (1 + np.exp(-delta_Z))
...
def Rank_Score_Calculate(RT_GT, Z_score, Pair):
    delta_Z = Z_score[Pair[:, 0]] - Z_score[Pair[:, 1]]
    delta_Z = np.clip(delta_Z, -50, 50)
    flag = 1 / (1 + np.exp(-delta_Z))
    Idx_flag = copy.deepcopy(flag)
    ...
    for n in range(flag.shape[0]):
        ...
        if RT_GT[Pair[n, 0]] > RT_GT[Pair[n, 1]]:
            temp_score[0, 0] = 1
        else:
            temp_score[0, 0] = -1
        ...
        Score[Pair[n, 0], :] = Score[Pair[n, 0], :] + (temp_score == 1) + 0.5 * (temp_score == 0)
